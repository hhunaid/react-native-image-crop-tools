...
open class CropView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate, CropRectViewDelegate {
    @objc var sourceUrl: String? {
        didSet {
            if let sourceUrl = self.sourceUrl, let url = URL(string: sourceUrl), let data = try? Data(contentsOf: url) {
                self.image = UIImage(data: data)
            }
        }
    }
    @objc var onImageCropPress: RCTBubblingEventBlock?
    ...
    open override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if zoomingView?.frame.contains(zoomedPoint) ?? false {
            return scrollView
        }
    ...
    open func fireEvent() {
        ...
        if let filePath = paths.first?.appendingPathComponent("\(UUID().uuidString).jpg") {
            do {
                try image.jpegData(compressionQuality: 1)?.write(to: filePath)
                let imageSize = image.size
                onImageCropPress?(["uri": filePath.absoluteString, "width": imageSize.width, "height": imageSize.height])
            }catch {
                return
            }
        }
